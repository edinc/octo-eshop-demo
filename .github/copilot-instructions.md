...
The `.devcontainer/` config starts only infrastructure (PostgreSQL ×3, Redis) and installs all dev tools. Services are run from the terminal.

```bash
# Database migrations (inside devcontainer)
...
# Start a service individually
npm run dev --workspace=services/user-service
```

Included tools: Node 20, Docker, GitHub CLI, kubectl, Helm, Terraform, Azure CLI.
Per-service `.env` files are auto-generated by `.devcontainer/setup.sh`.
